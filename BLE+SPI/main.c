...
//DDS SPI Settings
const uint8_t DDSCHIPSELECT = 14; //
const uint8_t DDSSYNCIO = 15;     //
...
//SPI DMA Setup
static nrfx_spim_xfer_desc_t DDSFTWREGISTER_desc = NRFX_SPIM_XFER_TX(DDSFTWREGISTER_buf, 1);
static nrfx_spim_xfer_desc_t DDSCONTROLREGISTER_desc = NRFX_SPIM_XFER_TX(DDSCONTROLREGISTER_buf, 1);
static nrfx_spim_xfer_desc_t LEFTFTW_desc = NRFX_SPIM_XFER_TX(LEFTFTW_buf, 4);
static nrfx_spim_xfer_desc_t STRAIGHTFTW_desc = NRFX_SPIM_XFER_TX(STRAIGHTFTW_buf, 4);
static nrfx_spim_xfer_desc_t RIGHTFTW_desc = NRFX_SPIM_XFER_TX(RIGHTFTW_buf, 4);

static bool flagLeft;
static bool flagStraight;
static bool flagRight;
...
void toggleHiLo(uint32_t GPIO) { // Make settings take effect.
  ...
  nrf_delay_us(1);  // Prevents hold time violation
...
void ddsFrequencySet(nrfx_spim_xfer_desc_t freqDesc){
  //Assert frequency tuning register
  ...
  //Send frequency value
    spi_xfer_done = false;
    nrfx_spim_xfer(&spi, &freqDesc, 0);
    while (!spi_xfer_done){
        __WFE();
    }

  //Apply changes
    toggleHiLo(DDSSYNCIO);
}


///////////////////////////////////////////END SPI & DDS STUFF till main////////////////////////////////////////////////////////////////
...
static void nus_data_handler(ble_nus_evt_t * p_evt)
{
    ...
    if (p_evt->type == BLE_NUS_EVT_RX_DATA)
    {
      ...
      //Data intercept, set flags for DDS
                if (p_evt->params.rx_data.p_data[i] == 65){
                 flagLeft = true;
                  }
                else if (p_evt->params.rx_data.p_data[i] == 66){
                 flagStraight = true;
                  }
                else if (p_evt->params.rx_data.p_data[i] == 67){
                flagRight = true;
                  }
...
static void idle_state_handle(void)
{
  ...
  //Assert frequency tuning register
   if(flagLeft){
      flagLeft = false;
      ddsFrequencySet(LEFTFTW_desc);
  }
   if(flagStraight){
      flagStraight = false;
      ddsFrequencySet(STRAIGHTFTW_desc);
  }
   if(flagRight){
      flagRight = false;
      ddsFrequencySet(RIGHTFTW_desc);
  }
}
...
int main(void)
{
    ...
    conn_params_init();
     ...
     //DDS & SPI Settings
      ...
      nrf_gpio_pin_write(DDSCHIPSELECT, 0);
